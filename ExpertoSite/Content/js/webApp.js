...
var webApp = function () {
    ...
    var _popupConfirmacionSintoma = "popupConfirmacionSintoma";
    ...
    var createMessageDeleteDialog = function () {
        ...
        dialogMessage += "</div>";
        dialogMessage += "<div class='modal-body'><p></p></div>";
        dialogMessage += "<div class='modal-footer' style='margin-top: 0px; margin-bottom: 0px;'>";
    ...
    var createConfirmDialogSintoma = function () {
        var dialogConfirm = "<div id='" + _popupConfirmacionSintoma + "' tabindex='-1' role='dialog' aria-hidden='true' class='modal fade' data-backdrop='static' style='z-index:100000;'>";
        dialogConfirm += "<div class='modal-dialog'>";
        dialogConfirm += "<div class='modal-content'>";
        dialogConfirm += "<div class='modal-header'><h4 class='modal-title'></h4></div>";
        dialogConfirm += "<div class='modal-body'><h4></h4></div>";
        dialogConfirm += "<div class='modal-footer' style='margin-top: 0px; margin-bottom: 0px;'>";
        dialogConfirm += "<button class='btn btn-primary' data-dismiss='modal'><i class='fa fa-remove'></i> NO</button> ";
        dialogConfirm += "<button class='btn btn-success' data-dismiss='modal'><i class='fa fa-thumbs-o-up'></i> SI</button>";
        dialogConfirm += "</div>";
        dialogConfirm += "</div>";
        dialogConfirm += "</div>";
    ...
    var validarLetrasEspacio = function(identificadores) {
        $.each(identificadores, function (index, item) {
            $('#'+item).validCampoFranz(' abcdefghijklmnñopqrstuvwxyzáéiou');
        });        
    }; 

    var validarAlfanumerico = function(identificadores) {
        $.each(identificadores, function (index, item) {
            $('#'+item).validCampoFranz(' abcdefghijklmnñopqrstuvwxyzáéiou1234567890');
        });        
    }; 
    ...
    return {
        ...
        showReConfirmDialog: function (fnSuccess, message, fnCancel) {
            ...
            if (message != null && message != '') {
                $('#' + _popupReConfirmacion + ' .modal-body p').text(message);
            } else {
                $('#' + _popupReConfirmacion + ' .modal-body p').text(_mensajePopupConfirmacion);
            }
        ...
        showConfirmDialogSintoma: function (title, message, fnSuccess, fnCancel) {
            
            var popup = $('#' + _popupConfirmacionSintoma);
            popup.modal('show');
            var btnSuccess = $(popup).find('.btn-success');
            var btnCancel = $(popup).find('.btn-primary');
            ...
            if (title != null && title != '') {
                $('#' + _popupConfirmacionSintoma + ' .modal-header h4').text(title);
            } else {
                $('#' + _popupConfirmacionSintoma + ' .modal-header h4').text(title);
            }            

            if (message != null && message != '') {
                $('#' + _popupConfirmacionSintoma + ' .modal-body h4').text(message);
            } else {
                $('#' + _popupConfirmacionSintoma + ' .modal-body h4').text(_mensajePopupConfirmacion);
            }
